...
fn posthog_options(config: &AnalyticsConfig) -> Option<ClientOptions> {
    ...
    options
        ...
        .disable_geoip(true)
        .is_server(false)
        .on_error(|error| log::warn!("Analytics delivery failed: {:?}", error));
    ...
    Some(
        options
            .build()
            .expect("Static analytics configuration must be valid"),
    )
}
...
impl AnalyticsClient {
    pub async fn new(config: AnalyticsConfig) -> Self {
        let client = match posthog_options(&config) {
            Some(options) => Some(Arc::new(posthog_rs::client(options).await)),
            None => None,
        };
    ...
    #[allow(clippy::too_many_arguments)] // One stable analytics event schema.
...
mod tests {
    ...
    #[test]
    fn analytics_transport_requires_explicit_opt_in_and_a_key() {
        assert!(posthog_options(&AnalyticsConfig::default()).is_none());
        assert!(posthog_options(&AnalyticsConfig {
            enabled: true,
            ..AnalyticsConfig::default()
        })
        .is_none());

        let options = posthog_options(&AnalyticsConfig {
            api_key: "test-project-key".to_string(),
            enabled: true,
            ..AnalyticsConfig::default()
        })
        .expect("explicit opt-in with a key should configure transport");
