...
mod tests {
    ...
    async fn test_core_audio_capture() {
        ...
        let sample_count = tokio::time::timeout(tokio::time::Duration::from_secs(15), async {
            let mut sample_count = 0;
            while sample_count < 48_000 {
        ...
        .await
        .expect("Timed out before collecting one second of system audio");

        info!("Collected {} samples", sample_count);
        assert!(sample_count >= 48_000);
    }
}
