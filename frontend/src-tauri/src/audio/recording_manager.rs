...
impl RecordingManager {
    ...
    async fn rollback_failed_start(&mut self) {
        ...
        if let Err(error) = self.stream_manager.stop_streams() {
            warn!(
                "Failed to stop streams while rolling back startup: {}",
                error
            );
        }
        ...
        if let Err(error) = self.pipeline_manager.stop().await {
            warn!(
                "Failed to stop pipeline while rolling back startup: {}",
                error
            );
        }

        self.recording_saver.discard_unstarted_recording();
    }
    ...
    pub async fn start_recording(
        ...
        if let Err(error) = self.state.start_recording() {
