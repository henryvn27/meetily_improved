...
impl RecordingSaver {
    ...
    /// Discard a recording session that never completed native startup.
    ///
    /// Only metadata-only folders are removed. Any audio checkpoint, finalized
    /// audio, or transcript is treated as recoverable user data and preserved.
    pub fn discard_unstarted_recording(&mut self) {
        if let Ok(mut is_saving) = self.is_saving.lock() {
            *is_saving = false;
        }
        ...
        if Self::folder_contains_recoverable_data(&folder) {
            warn!(
                "Preserving failed-start meeting folder because it contains recoverable data: {}",
                folder.display()
            );
            return;
        }
        ...
        match std::fs::remove_dir_all(&folder) {
            Ok(()) => {
                info!(
                    "Removed metadata-only meeting folder after recording startup failed: {}",
                    folder.display()
                );
            ...
            Err(error) => warn!(
                "Failed to remove metadata-only meeting folder {}: {}",
                folder.display(),
                error
            ),
        }
    ...
    fn folder_contains_recoverable_data(folder: &std::path::Path) -> bool {
        if folder.join("audio.mp4").is_file() || folder.join("transcripts.json").is_file() {
            return true;
        }
...
mod tests {
    ...
    #[test]
    fn failed_start_removes_metadata_only_folder() {
        let folder = unique_test_folder("empty");
        std::fs::create_dir_all(folder.join(".checkpoints")).unwrap();
        std::fs::write(folder.join("metadata.json"), "{}").unwrap();

        let mut saver = RecordingSaver::new();
        saver.meeting_folder = Some(folder.clone());
        saver.discard_unstarted_recording();
    ...
    #[test]
    fn failed_start_preserves_folder_with_audio_checkpoint() {
        let folder = unique_test_folder("checkpoint");
        std::fs::create_dir_all(folder.join(".checkpoints")).unwrap();
        std::fs::write(folder.join("metadata.json"), "{}").unwrap();
        std::fs::write(folder.join(".checkpoints/chunk-0001.wav"), [1_u8]).unwrap();
        ...
        assert!(folder.exists());
        assert_eq!(saver.meeting_folder.as_ref(), Some(&folder));
